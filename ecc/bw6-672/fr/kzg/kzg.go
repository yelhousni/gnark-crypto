...
import (
	...
	"github.com/consensys/gnark-crypto/ecc/bw6-672"
...
type Digest = bw6672.G1Affine

// SRS stores the result of the MPC
type SRS struct {
	G1 []bw6672.G1Affine  // [gen [alpha]gen , [alpha**2]gen, ... ]
	G2 [2]bw6672.G2Affine // [gen, [alpha]gen ]
}
...
func NewSRS(size uint64, bAlpha *big.Int) (*SRS, error) {
	...
	var srs SRS
	srs.G1 = make([]bw6672.G1Affine, size)

	var alpha fr.Element
	alpha.SetBigInt(bAlpha)

	_, _, gen1Aff, gen2Aff := bw6672.Generators()
	srs.G1[0] = gen1Aff
	srs.G2[0] = gen2Aff
	...
	g1s := bw6672.BatchScalarMultiplicationG1(&gen1Aff, alphas)
	copy(srs.G1[1:], g1s)
...
func Verify(commitment *Digest, proof *OpeningProof, srs *SRS) error {
	...
	// comm(f(a))
	var claimedValueG1Aff bw6672.G1Affine
	...
	// [f(alpha) - f(a)]G1Jac
	var fminusfaG1Jac, tmpG1Jac bw6672.G1Jac
	fminusfaG1Jac.FromAffine(commitment)
	tmpG1Jac.FromAffine(&claimedValueG1Aff)
	fminusfaG1Jac.SubAssign(&tmpG1Jac)

	// [-H(alpha)]G1Aff
	var negH bw6672.G1Affine
	negH.Neg(&proof.H)

	// [alpha-a]G2Jac
	var alphaMinusaG2Jac, genG2Jac, alphaG2Jac bw6672.G2Jac
	...
	// [alpha-a]G2Aff
	var xminusaG2Aff bw6672.G2Affine
	xminusaG2Aff.FromJacobian(&alphaMinusaG2Jac)

	// [f(alpha) - f(a)]G1Aff
	var fminusfaG1Aff bw6672.G1Affine
	fminusfaG1Aff.FromJacobian(&fminusfaG1Jac)

	// e([-H(alpha)]G1Aff, G2gen).e([-H(alpha)]G1Aff, [alpha-a]G2Aff) ==? 1
	check, err := bw6672.PairingCheck(
		[]bw6672.G1Affine{fminusfaG1Aff, negH},
		[]bw6672.G2Affine{srs.G2[0], xminusaG2Aff},
	)
	if err != nil {
		return err
	}
...
func BatchVerifyMultiPoints(digests []Digest, proofs []OpeningProof, srs *SRS) error {
	...
	var foldedQuotients bw6672.G1Affine
	quotients := make([]bw6672.G1Affine, len(proofs))
	...
	var foldedEvalsCommit bw6672.G1Affine
	...
	var foldedPointsQuotients bw6672.G1Affine
	for i := 0; i < len(randomNumbers); i++ {
	...
	// pairing check
	check, err := bw6672.PairingCheck(
		[]bw6672.G1Affine{foldedDigests, foldedQuotients},
		[]bw6672.G2Affine{srs.G2[0], srs.G2[1]},
	)
	if err != nil {
		return err
	}
