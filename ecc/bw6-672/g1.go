...
func (p *G1Jac) ClearCofactor(a *G1Jac) *G1Jac {
	var uP, vP, wP, L0, L1, tmp, htP G1Jac
	var t, one, v, d1, d2, d3, ht big.Int
	d1.SetInt64(6532624810000)
	d2.SetInt64(2555900)
	d3.SetInt64(2555900)
	ht.SetInt64(5111800)
	one.SetInt64(1)
	v.Mul(&xGen, &xGen).Add(&v, &one)
	t.Add(&xGen, &one).Neg(&t)
	v.Mul(&v, &t)

	htP.ScalarMultiplication(a, &ht)
	uP.ScalarMultiplication(a, &xGen).Neg(&uP)
	vP.Set(&uP).SubAssign(a).
		ScalarMultiplication(&vP, &v)
	wP.ScalarMultiplication(&vP, &xGen).Neg(&wP).
		AddAssign(&vP).
		AddAssign(&uP)

	L0.ScalarMultiplication(&wP, &d1)
	tmp.ScalarMultiplication(&vP, &d2)
	L0.AddAssign(&tmp).AddAssign(&htP)

	L1.Set(a).AddAssign(&uP).
		ScalarMultiplication(&L1, &d1)
	tmp.Set(a).AddAssign(&vP).
		ScalarMultiplication(&tmp, &d3)
	L1.SubAssign(&tmp).AddAssign(&htP)
