...
func (p *G2Jac) ClearCofactor(a *G2Jac) *G2Jac {
	var uP, u2P, u3P, u4P, u5P, vP, wP, L0, L1, tmp, htP G2Jac
	...
	vP.Set(&uP).AddAssign(&u3P).
		AddAssign(&u2P).
		Double(&vP).
		AddAssign(&u4P).
		AddAssign(a)
	wP.Set(&uP).SubAssign(&u4P).
		SubAssign(&u5P)
	htP.ScalarMultiplication(a, &ht)

	L0.Set(&uP).SubAssign(a).
		ScalarMultiplication(&L0, &d1)
	tmp.ScalarMultiplication(&vP, &d3)
	L0.SubAssign(&tmp).SubAssign(&htP)

	L1.ScalarMultiplication(&wP, &d1)
	tmp.Set(a).AddAssign(&vP).
		ScalarMultiplication(&tmp, &d2)
	L1.AddAssign(&tmp).SubAssign(&htP)
