...
// Encoder writes cp8-632 object values to an output stream
type Encoder struct {
	w   io.Writer
...
// Decoder reads cp8-632 object values from an inbound stream
type Decoder struct {
	r io.Reader
	n int64 // read bytes
}

// NewDecoder returns a binary decoder supporting curve cp8-632 objects in both
...
func (dec *Decoder) Decode(v interface{}) (err error) {
	...
	if rv.Kind() != reflect.Ptr || rv.IsNil() || !rv.Elem().CanSet() {
		return errors.New("cp8-632 decoder: unsupported type, need pointer")
	}
	...
	default:
		n := binary.Size(t)
		if n == -1 {
			return errors.New("cp8-632 encoder: unsupported type")
		}
...
// NewEncoder returns a binary encoder supporting curve cp8-632 objects
...
const SizeOfG2AffineCompressed = 80 * 2
...
func (p *G2Affine) RawBytes() (res [SizeOfG2AffineUncompressed]byte) {
	...
	binary.BigEndian.PutUint64(res[272:280], tmp[5])
	binary.BigEndian.PutUint64(res[264:272], tmp[6])
	binary.BigEndian.PutUint64(res[256:264], tmp[7])
	...
	binary.BigEndian.PutUint64(res[192:200], tmp[5])
	binary.BigEndian.PutUint64(res[184:192], tmp[6])
	binary.BigEndian.PutUint64(res[176:184], tmp[7])
