...
func (p *G2Jac) ClearCofactor(a *G2Jac) *G2Jac {
	var uP, u2P, u3P, htP, L0, L1, tmp G2Jac
	var d1, d2, d3, ht big.Int
	d1.SetInt64(163)
	d2.SetInt64(8)  // negative
	d3.SetInt64(17)  // negative
	ht.SetInt64(25) // negative
	...
	L0.Set(a).SubAssign(&uP).
		SubAssign(&uP).
		AddAssign(&u2P).
		ScalarMultiplication(&L0, &d3)
	tmp.Set(a).AddAssign(&uP).
		ScalarMultiplication(&tmp, &d1)
	L0.SubAssign(&tmp).
		AddAssign(&htP)
	L1.Set(&u3P).SubAssign(&u2P).
		SubAssign(&uP).
		ScalarMultiplication(&L1, &d1)
	tmp.Set(a).SubAssign(&uP).
        Double(&tmp).
		AddAssign(&u2P).
		ScalarMultiplication(&tmp, &d2)
	L1.SubAssign(&tmp).
		AddAssign(&htP)
